...
#endif
$output v_color0, v_color1, v_fog, v_refl, v_texcoord0, v_lightmapUV, v_extra, v_worldPos, v_position
...
void main() {
  ...
  vec4 worldTime = worldTimeDetection(FogColor.rgb, FogAndDistanceControl.xyz);
  
  vec4 fogColor;
  fogColor.rgb = nlRenderSky(horizonEdgeCol, horizonCol, zenithCol, viewDir, FogColor.rgb, t, rainFactor, end, underWater, nether);
  fogColor.a = nlRenderFogFade(relativeDist, FogColor.rgb, FogAndDistanceControl.xyz);
  #ifdef NL_GODRAY 
  ...
  if (nether) {
    ...
    float cloudyFogAlpha = mix(mix(NL_CLOUDY_FOG_DAY, NL_CLOUDY_FOG_DAWN, worldTime.x), NL_CLOUDY_FOG_NIGHT, worldTime.z);
  
    float fg = smoothstep(0.0, 1.0-cloudyFogAlpha, relativeDist);
